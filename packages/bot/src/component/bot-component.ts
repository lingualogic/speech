...
import { DIALOG_COMPONENT_NAME, OnDialogSetFunc, OnDialogJsonFunc, OnDialogParseFunc, OnDialogStartFunc, OnDialogStopFunc, OnDialogStateSetFunc, OnDialogActionFunc, OnDialogActionStopFunc, OnDialogSpeakFunc, OnDialogSpeakStartFunc, OnDialogSpeakStopFunc, DialogActionInterface, DialogSpeakInterface, DialogComponentInterface, DialogDataInterface } from '@speech/dialog';


// bot

import { BOT_API_VERSION, BOT_VERSION_STRING } from '../bot-version';
...
export class BotComponent extends BaseComponent implements BotComponentInterface {
    ...
    private mSpeak: SpeakComponentInterface = null;
    private mListen: ListenComponentInterface = null;
    private mAction: ActionComponentInterface = null;
    private mDialog: DialogComponentInterface = null;

    // Komponenten-Flags zum ein/ausschalten der Komponenten in Bot

    private mSpeakEnableFlag = true;
    private mListenEnableFlag = true;
    private mActionEnableFlag = true;
    ...
    constructor( aComponentName = '', aRegisterFlag = true ) {
        super( aComponentName || BOT_COMPONENT_NAME, aRegisterFlag );
    }
    ...
    protected _initAllPlugin(): number {
        ...
        if ( !this.mDialog ) {
            this.error( '_initAllPlugin', 'keine Dialog-Komponente vorhanden' );
            this._clearInit();
            return -1;
        }
        return 0;
    ...
    protected _dialogSpeak( aDialogSpeak: DialogSpeakInterface ): number {
        if ( this.isActive() && this.isSpeak()) {
            // Mappen der DialogSpeak-Daten auf die Speak-Daten
            this.mSpeak.setText( aDialogSpeak.text );
    ...
    protected _dialogSpeakStop(): number {
        if ( this.isActive() && this.isSpeak()) {
    ...
    protected _dialogAction( aDialogAction: DialogActionInterface ): number {
        if ( this.isActive() && this.isAction()) {
            ...
            return this.mAction.startAction( action );
        }
        return 0;
    }
    ...
    protected _dialogActionStop(): number {
    ...
    getDialogFilePath(): string {
        ...
        if ( !this.mDialog ) {
            this.error( 'getDialogFilePath', 'keine Dialog-Komponente vorhanden' );
            return '';
        }
    ...
    getDialogFileName(): string {
        // console.log('BotComponent.getDialogFilePath');
        if ( !this.mDialog ) {
            this.error( 'getDialogFileName', 'keine Dialog-Komponente vorhanden' );
            return '';
        }
    ...
    getDialog(): string {
        ...
        if ( !this.mDialog ) {
            this.error( 'getDialog', 'keine Dialog-Komponente vorhanden' );
            return '';
        }
    ...
    isRunning(): boolean {
        if ( !this.mDialog ) {
            ...
            // this.error( 'isRunning', 'keine Dialog-Komponente vorhanden' );
            return false;
        }
    ...
    getDialogState(): string {
        ...
        if ( !this.mDialog ) {
            this.error( 'getDialogState', 'keine Dialog-Komponente vorhanden' );
            return '';
        }
