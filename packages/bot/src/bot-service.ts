...
import { DialogActionInterface, DialogSpeakInterface } from '@speech/dialog';


// bot

import { BOT_COMPONENT_NAME, BOT_SERVICE_NAME } from './bot-const';
...
import { BotDataInterface } from './bot-data.interface';
import { BotOptionInterface } from './bot-option.interface';
import { BotServiceConfig } from './bot-service-config';
import { BotServiceInterface } from './bot-service.interface';
...
export type BotServiceActionStartFunc = (aAction: BotActionInterface) => void;
...
export class BotService extends Service implements BotServiceInterface {
    ...
    private mDialogImportEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
    private mDialogParseEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
    private mDialogStartEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
    private mDialogStopEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
    private mDialogStateSetEvent = new EventEmitter<string>( BOT_ASYNC_EVENT );
    private mDialogActionEvent = new EventEmitter<BotActionInterface>( BOT_ASYNC_EVENT );
    private mDialogActionStopEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
    private mDialogSpeakEvent = new EventEmitter<string>( BOT_ASYNC_EVENT );
    private mDialogSpeakStopEvent = new EventEmitter<any>( BOT_ASYNC_EVENT );
     ...
     *
     * @static
     * @return BotServiceOptions - dient zur Einstellung der otionalen Parameter
     */

    static getConfig(): BotOptionInterface {
    ...
    constructor() {
        super( BOT_COMPONENT_NAME, BOT_SERVICE_NAME, BOT_API_VERSION );
        ...
        if ( ServiceManager.insert( this ) !== 0 ) {
            console.log('BotService: wurde nicht in ServiceManager eingetragen');
        }
    }
    ...
    protected _setOption( aOption: BotOptionInterface ): number {
    ...
    protected _addAllEvent( aServiceName: string ): number {
        ...
        this.mBot.addDialogActionEvent( aServiceName, (aAction: DialogActionInterface) => {
            ...
            const action: BotActionInterface = {
                state: aAction.state,
    ...
    import( aJsonDialogData: BotDataInterface[]): number {
        if ( !this.mBot ) {
