...
export class DialogComponentBuilder extends Builder {
    ...
    build( aConfig?: BuilderConfigInterface ): DialogComponentInterface {
        ...
        let dialog = this._findComponent( componentName ) as DialogComponentInterface;
        if ( dialog ) {
            return dialog;
        }
        ...
        try {
            ...
            const fileReader = this._getPlugin( FILEREADER_PLUGIN_NAME, FILEREADER_PLUGIN_NAME, FILEREADER_FACTORY_NAME, FileReaderFactory ) as FileReaderInterface;
            const store = this._getPlugin( STORE_PLUGIN_NAME, STORE_PLUGIN_NAME, STORE_FACTORY_NAME, StoreFactory ) as StoreInterface;
            const json = this._getPlugin( JSON_PLUGIN_NAME, JSON_PLUGIN_NAME, JSON_FACTORY_NAME, JsonFactory ) as JsonInterface;
            const parser = this._getPlugin( PARSER_PLUGIN_NAME, PARSER_PLUGIN_NAME, PARSER_FACTORY_NAME, ParserFactory ) as ParserInterface;
            const interpreter = this._getPlugin( INTERPRETER_PLUGIN_NAME, INTERPRETER_PLUGIN_NAME, INTERPRETER_FACTORY_NAME, InterpreterFactory ) as InterpreterInterface;
    ...
    protected _buildComponent( aConfig: BuilderConfigInterface ): DialogComponentInterface {
        const componentName = this._getComponentName( aConfig ) || DIALOG_COMPONENT_NAME;
        const componentClass = this._getComponentClass( aConfig ) || DIALOG_COMPONENT_NAME;
        return this._getPlugin( componentName, componentClass, DIALOG_COMPONENTFACTORY_NAME, DialogComponentFactory ) as DialogComponentInterface;
    }
    ...
    protected _binder( aDialog: DialogComponentInterface, aFileReader: FileReaderInterface, aStore: StoreInterface, aJson: JsonInterface, aParser: ParserInterface, aInterpreter: InterpreterInterface ): number {
        if ( !aDialog ) {
