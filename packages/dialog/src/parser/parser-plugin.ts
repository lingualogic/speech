...
import { IDialogObject } from '../store/dialog-object.interface';
import { StoreNewDialogFunc, StoreNewDialogStateFunc } from '../store/store.interface';
import { IParser, OnParserEndFunc, ParserSpeechDefFileFunc, ParserSpeechDefDataFunc } from './parser.interface';
...
export class ParserPlugin extends Plugin implements IParser {

    // Event-Funktionen

    private mOnParserEndFunc: OnParserEndFunc = null;
    ...
    constructor( aPluginName = '', aRegisterFlag = true ) {
        super( aPluginName || PARSER_PLUGIN_NAME, aRegisterFlag );
    ...
    protected _onParserEnd(): number {
        ...
        if ( typeof this.mOnParserEndFunc === 'function' ) {
            try {
                ...
                this.exception( '_onParserEnd', aException );
                return -1;
            }
        }
        return 0;
     ...
     *
     * @return {IDialogObject} Rueckgabe des Dialogdatenobjektes
     */

    protected _newDialog( aDialogName: string ): IDialogObject {
     ...
     *
     * @return {IDialogState} Rueckgabe des Dialogzustandsdatenobjektes
     */

    protected _newDialogState( aDialogName: string, aStateName: string, aStateId: number ): IDialogState {
    ...
    parseSpeechDefFile( aDefFileName: string ): number {
        ...
        if ( !aDefFileName ) {
            this.error( 'parseSpeechDefFile', 'kein Dateiname uebergeben' );
            return -1;
        }
        try {
            ...
            this.error( 'parseSpeechDefFile', 'nicht implementiert' );
            return 0;
        } catch ( aException ) {
            this.exception( 'parseSpeechDefFile', aException );
            return -1;
        }
    }
    ...
    parseSpeechDefData( aDefData: string ): number {
        ...
        if (!aDefData) {
            this.error( 'parseSpeechDefData', 'keine Def-Daten uebergeben' );
            return -1;
        }
        ...
        let token = '';
        let dialogName = '';
        let dialogObject: IDialogObject = null;
        let stateObject: IDialogState = null;
        let nodeObject: IDialogNode = null;
        let stateIdCount = 0;
        let nodeIdCount = 0;
        let stateId = 0;
        let firstNodeId = 0;
            ...
            for ( let index = 0; index < speechDefArray.length; ++index ) {
                ...
                switch ( token ) {
                    ...
                    default:
                        ...
                        this.error( 'parseSpeechDefData', 'ParserFehler' );
                        return -1;
                        // break;
                }
