...
import { IDialogData, IDialogCommand } from '../dialog-data.interface';
...
import { IJson, OnJsonEndFunc, TransformJsonFileFunc, TransformJsonDataFunc } from './json.interface';
...
export class JsonPlugin extends Plugin implements IJson {


    private mJsonStore: JsonStore = null;
    private mDialogObject: IDialogObject = null;
    private mStateObject: IDialogState = null;
    ...
    protected _transformCommand( aStateId: number, aCommand: IDialogCommand, aEndFlag: boolean ): number {
        try {
            this.mNodeIdCount++;
            const id = this.mNodeIdCount;
            if ( this.mFirstNodeId === 0 ) {
    ...
    getTransformJsonDataFunc(): TransformJsonDataFunc {
        return (aDialogData: IDialogData[]) => {
