...
import { IDialogData, IDialogState, IDialogIntent, IDialogText, IDialogCommand } from './../dialog-data.interface';
...
export class JsonStore extends ErrorBase {
    ...
    private mDialogList: IDialogData[] = null;
    private mDialog: IDialogData = null;
    private mDialogName = '';

    private mStateList: IDialogState[] = null;
    private mState: IDialogState = null;
    private mStateName = '';

    private mIntentList: IDialogIntent[] = null;
    private mIntent: IDialogIntent = null;
    private mIntentName = '';

    private mCommandList: IDialogCommand[] = null;

    private mTextList: IDialogText[] = null;
    private mText: IDialogText = null;
    private mTextName = '';

    // feste Namenslisten

    private mCommandNameList = [ DIALOG_ACTION_COMMAND, DIALOG_SPEAK_COMMAND, DIALOG_WAIT_COMMAND ];
    ...
    setDialogList( aDialogList: IDialogData[]): number {
        if ( !aDialogList || aDialogList.length === 0 ) {
            this.error( 'setDialogList', 'keine Dialogliste uebergeben' );
            return -1;
        }
        this.clear();
    ...
    setCurrentDialog( aDialogName: string ): number {
        if ( !aDialogName ) {
            this.error( 'setCurrentDialog', 'kein Dialogname uebergeben' );
            return -1;
        }
        if ( !this.mDialogList ) {
            this.error( 'setCurrentDialog', 'keine Dialogliste vorhanden' );
            return -1;
        }
        ...
        this.mText = null;
        this.mTextName = '';
        this.error( 'setCurrentDialog', 'kein Dialog zum Dialognamen ' + aDialogName + ' gefunden');
        return -1;
    }


    set dialogList( aDialogList: IDialogData[]) {
        this.mDialogList = aDialogList;
    }
    ...
    newState( aStateName: string ): number {
        if ( !this.mDialog || !this.mStateList ) {
            this.error( 'newState', 'kein Dialog vorhanden' );
            return -1;
        }
        if ( this.checkState( aStateName )) {
            this.error( 'newState', 'kein State Name uebergeben' );
            return -1;
        }
        // State erzeugen
        const newState = {
    ...
    deleteState( aStateName: string ): number {
        ...
        if ( !this.mDialog || !this.mStateList ) {
            this.error( 'deleteState', 'kein Dialog vorhanden' );
            return -1;
        }
        if ( !this.checkState( aStateName )) {
            this.error( 'deleteState', 'kein State vorhanden' );
            return -1;
        }
        // State entfernen
    ...
    setCurrentState( aStateName: string ): number {
        ...
        this.mIntent = null;
        this.mIntentName = '';
        this.error( 'setCurrentState', 'kein State vorhanden' );
        return -1;
    }
    ...
    newStateIntent( aIntentName: string ): number {
        if ( !aIntentName ) {
            this.error( 'newStateIntent', 'kein Intent Name uebergeben' );
            return -1;
        }
        if ( !this.mState ) {
            this.error( 'newStateIntent', 'kein State vorhanden' );
            return -1;
        }
   ...
   */
    ...
    deleteStateIntent( aIntentName: string ): number {
        ...
        if ( !this.mState ) {
            this.error( 'deleteStateIntent', 'kein State vorhanden' );
            return -1;
        }
        if ( !this.checkStateIntent( aIntentName )) {
            this.error( 'deleteStateIntent', 'kein Intent vorhanden' );
            return -1;
        }
        // Intent entfernen
    ...
    setCurrentIntent( aIntentName: string ): number {
        ...
        this.mIntentName = '';
        this.mCommandList = null;
        this.error( 'setCurrentIntent', 'kein Intent vorhanden' );
        return -1;
    }
    ...
    newCommand( aCommandName: string ): number {
        if ( !aCommandName ) {
            this.error( 'newCommand', 'kein Kommandoname uebergeben' );
            return -1;
        }
        if ( !this.mIntent ) {
            this.error( 'newCommand', 'kein Intent vorhanden' );
            return -1;
        }
        // Kommando in Intent eintragen
        const command: IDialogCommand = {
            name: aCommandName
        };
    ...
    deleteCommand( aCommandIndex: number ): number {
        ...
        if ( !this.mIntent ) {
            this.error( 'deleteCommand', 'kein Intent vorhanden' );
            return -1;
        }
        // Command entfernen
    ...
    setCurrentText( aTextName: string ): number {
        ...
        this.error( 'setCurrentText', 'kein Text gefunden' );
        return -1;
    }
    ...
    newText( aTextName: string ): number {
        if ( !aTextName ) {
            this.error( 'newText', 'kein Text Name uebergeben' );
            return -1;
        }
        if ( !this.mTextList ) {
            this.error( 'newText', 'keine Textliste vorhanden' );
            return -1;
        }
        const text: IDialogText = {
            name: aTextName,
            text: '',
            time: 0
        };
    ...
    deleteText( aTextId: string ): number {
        ...
        if ( !this.mTextList ) {
            this.error( 'deleteText', 'keine Textliste vorhanden' );
            return -1;
        }
    ...
    setText( aTextId: string, aText: string, aTime: number ): number {
        if ( !aTextId || !aText || !this.mTextList ) {
            this.error( 'setText', 'keine Textdaten uebergeben' );
            return -1;
        }
        for ( const text of this.mTextList ) {
