...
import { SPEECH_DIALOGACTION_EVENT, SPEECH_DIALOGSPEAK_EVENT, Plugin } from '@lingualogic-speech/core';
...
import { IDialogNode } from '../store/dialog-node.interface';
import { DialogNode } from '../store/dialog-node';
import { IDialogState } from '../store/dialog-state.interface';
...
export class InterpreterPlugin extends Plugin implements IInterpreter {

    private mDialogName = DIALOG_MAIN_NAME;
    private mStateName = DIALOG_ROOTSTATE_NAME;
    private mStateContext: IDialogStateContext = null;
    // TODO: Typensichere Promise mit cancel() als Erweiterung funktioniert noch nicht
    private mNodePromise: any = null;
    private mDialogRunFlag = false;
    private mSpeakRunFlag = false;
    // Gruppen-Attribute
    private mGroupId = 0;
    ...
    // Funktionen
    private mGetDialogStateFunc: StoreGetDialogStateFunc = null;
    ...
    constructor( aPluginName = '', aRegisterFlag = true ) {
        super( aPluginName || INTERPRETER_PLUGIN_NAME, aRegisterFlag );
    ...
    protected _onDialogSet( aDialogName: string ): number {
        if ( typeof this.mDialogSetFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogSet', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogStart(): number {
        if ( typeof this.mDialogStartFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogStart', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogStop(): number {
        if ( typeof this.mDialogStopFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogStop', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogStateSet( aStateName: string ): number {
        if ( typeof this.mDialogStateSetFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogStateSet', aException );
                return -1;
            }
        }
        return 0;
     ...
     *
     * @private
     * @param {IDialogNode} aNode - Actionknoten
     *
     * @return {number} errorcode (0,-1)
     */

    protected _onDialogAction( aNode: IDialogNode ): number {
        if ( typeof this.mDialogActionFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogAction', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogSpeak( aNode: IDialogNode ): number {
        if ( typeof this.mDialogSpeakFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogSpeak', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogSpeakStart(): number {
        if ( typeof this.mDialogSpeakStartFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogSpeakStart', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onDialogSpeakStop(): number {
        if ( typeof this.mDialogSpeakStopFunc === 'function' ) {
            try {
                ...
                this.exception( '_onDialogSpeakStop', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _getDialogState( aDialogName: string, aStateName: string ): IDialogState {
    ...
    setDialog( aDialogName: string ): number {
        ...
        if ( !aDialogName ) {
            this.error( 'setDialog', 'kein Dialogname uebergeben' );
            return -1;
        }
    ...
    startDialog(): number {
        try {
            ...
            if ( this.isDialogRunning()) {
                this.error( 'startDialog', 'Dialog laeuft bereits' );
                return -1;
            }
            const dialogState = this._getDialogState( this.mDialogName, this.mStateName );
            if ( !dialogState ) {
                this.error( 'startDialog', 'kein DialogState vorhanden' );
                return -1;
            }
            ...
            this.exception( 'startDialog', aException );
            return -1;
        }
    }
    ...
    stopDialog(): number {
        ...
        try {
            ...
            this.exception( 'stopDialog', aException );
            return -1;
        }
    }
     ...
     * @param {IDialogStateContext} aStateContext - Zustandskontextobjekt
     *
     * @return {number} errorcode(0,-1) - Fehlercode
     */
    ...
    setState( aStateName: string, aStateContext: IDialogStateContext ): number {
        ...
        if ( !aStateName ) {
            this.error( 'setState', 'kein StateName uebergeben' );
            return -1;
        }
     ...
     *
     * @param {IDialogStateContext} aStateContext - Dialog Zustandskontxtobjekt
     *
     * @return {number} errorcode(0,-1)
     */
    ...
    protected _setDialogRunFlag(): number {
     ...
     *
     * @private
     * @return {IDialogNode} waitNode
     */

    protected _getWaitNode(): IDialogNode {
    ...
    protected _setNodePromise( aNodePromise: Promise<any> ): void {
    ...
    protected _clearNodePromise(): void {
    ...
    skipNextSpeakNode(): number {
        ...
        if ( !this.mNodePromise ) {
            this.error( 'skipNextSpeakNode', 'keine Knoten-Promise vorhanden' );
            return -1;
        }
        // Promise abbrechen
        try {
            ...
            this.exception( 'skipNextSpeakNode', aException );
            return -1;
        }
    }
     ...
     *
     * @private
     * @param {IDialogState} aDialogState - Dialog Zustandsobjekt
     */

    protected async _runState( aDialogState: IDialogState ) {
        // console.log('_runState: start ', aDialogState);
        const nodeId = aDialogState.getFirstDialogNodeId();
        if ( !nodeId ) {
            this.error( 'runState', 'kein Knoten vorhanden' );
            return;
        }
        ...
        while ( node && this.mDialogRunFlag ) {
            ...
            try {
                ...
                this.exception( '_runState', aException );
            }
            ...
            if ( !nextId ) {
                ...
                if ( !node ) {
                    this.error( '_runState', 'kein Knoten zur Knoten-ID vorhanden' );
                }
            }
        }
        this._clearSpeakRunFlag();
        ...
        const promise = new Promise<void>(( resolve, reject ) => {
            ...
            const timeoutId = setTimeout(() => {
                ...
                try {
                    ...
                    this.exception( '_runAsyncNode', aException );
                    this._clearNodePromise();
                    reject( aException );
                }
            }, aTimeout );
     ...
     *
     * @private
     * @param {IDialogNode} aNode - Knoten Datenobjekt
     */

    protected _runNode( aNode: IDialogNode ): void {
        ...
        switch ( aNode.getNodeType()) {
            ...
            case DIALOG_WAIT_NODE:
                ...
                break;
            default:
                this.error( '_runNode', 'kein gueltiger Knoten ' + aNode.getNodeId());
                break;
        }
    }
    ...
    protected _runGroup( aNode: IDialogNode ): void {
    ...
    protected _isGroupProperty( aNodeProperty: any, aNodeObjectName: string ): boolean {
     ...
     *
     * @private
     * @param {IDialogNode} aNode - Knotenobjekt
     *
     * @return {boolean} nodeRunFlag - true, wenn Knoten ausgefuehrt werden kann
     */

    protected _checkRunNode( aNode: IDialogNode ): boolean {
    ...
    protected _runAction( aNode: IDialogNode ): void {
     ...
     *
     * @private
     * @param {IDialogNode} aNode - Sprachknoten Datenobjekt
     */

    protected _runSpeak( aNode: IDialogNode ): void {
        // console.log('InterpreterPlugin._runSpeak:', aNode.getTimeout(), aNode.getText());
        if ( this.isSpeakRunning()) {
            this.error( '_runSpeak', 'Speak laeuft bereits' );
            return;
        }
    ...
    protected _runWait( aNode: IDialogNode ): void {
