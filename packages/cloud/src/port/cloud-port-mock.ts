 ...
 * CloudPortMock zum Testen des Cloud-Service mit dem Framework
 *
...
import { Port, PortTransaction } from '@lingualogic-speech/core';
...
/**
 * Definiert die CloudPortMock-Klasse
 */

export class CloudPortMock extends Port {
    ...
    constructor( aPortName?: string, aRegisterFlag = true ) {
        super( aPortName || CLOUD_MOCK_NAME, aRegisterFlag );
    }
    ...
    init( aOption?: any ): number {
        ...
        if ( this.isErrorOutput()) {
            if ( this.googleNLUFlag ) {
                console.log('cloudPortMock: NLU ist vorhanden');
            } else {
                console.log('cloudPortMock: NLU ist nicht vorhanden');
            }
            if ( this.googleTTSFlag ) {
                console.log('cloudPortMock: TTS ist vorhanden');
            } else {
                console.log('cloudPortMock: TTS ist nicht vorhanden');
            }
            if ( this.googleASRFlag ) {
                console.log('cloudPortMock: ASR ist vorhanden');
            } else {
                console.log('cloudPortMock: ASR ist nicht vorhanden');
            }
        }
        this._onInit( 0 );
     ...
     *
     * @param {ICloudPortConfigData} aConfigData - Konfigurationsdaten { appKey: '', appId: '', nluTag: ''}
     *
    ...
    setConfig( aConfigData: ICloudPortConfigData ): number {
        if ( this.mDynamicCredentialsFlag ) {
            ...
            try {
                ...
                this.googleAppKey = aConfigData.appKey;
                this.googleServerUrl = aConfigData.serverUrl;
                this.dialogflowTokenServerUrl = aConfigData.tokenServerUrl;
                this.dialogflowProjectId = aConfigData.projectId;
                this.dialogflowSessionId = aConfigData.sessionId;
                this.dialogflowEnvironmentName = aConfigData.environmentName;
                /****
    ...
    getConfig(): ICloudPortConfigData {
        const configData: ICloudPortConfigData = {
            ...
            appKey: this.googleAppKey,
            serverUrl: this.googleServerUrl,
            tokenServerUrl: this.dialogflowTokenServerUrl,
            projectId: this.dialogflowProjectId,
            sessionId: this.dialogflowSessionId,
            environmentName: this.dialogflowEnvironmentName
    ...
    start( aPluginName: string, aAction: string, aOption?: any ): number {
        ...
        switch ( aAction ) {
            ...
            case CLOUD_ASRNLU_ACTION:
                ...
                result = this._startASR( this.mTransaction, option.language || CLOUD_DEFAULT_LANGUAGE, option.audioURL || '', true, option.useProgressive || false );
                break;
            case CLOUD_ASR_ACTION:
    ...
    _startNLU( aTransaction: PortTransaction, aText: string, aLanguage: string ): number {
        ...
        try {
            ...
            console.log('cloudPortMock._startNLU: _onResult wird aufgerufen');
