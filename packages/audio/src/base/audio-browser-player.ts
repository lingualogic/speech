 ...
 * AudioBrowserPlayer fuer Abspielen von Sprachdaten im Browser
 *
...
export class AudioBrowserPlayer extends ErrorBase {

    private mAudioContext: any = null;
    private mAudioCodec: AudioCodec = null;
    private mResampler: AudioResampler = null;

    private mOnAudioStartFunc = null;
    private mOnAudioStopFunc = null;

    private mOnAudioEndFunc = null;

    private mAudioSource = null;
    private mAudioArray = [];
    private mQueue = [];
    private mBeginSpeakFlag = true;
    private mAudioStopFlag = false;

    private mAudioBuffer = null;
    ...
    playByStreamOld( aOptions: any, aAudioArray: any ): void {
        try {
            ...
            this.exception( 'playByStream', aException );
        }
    }
        ...
        try {
            ...
            if ( this.mAudioCodec.findPcmCodec( aOptions.codec )) {
                ...
                */
                ...
                this.error( 'decode', 'Kein Decoder vorhanden fuer ' + aOptions.codec );
            }
        } catch ( aException ) {
            this.exception( 'decode', aException );
        }
    }
    ...
    protected _playStart( aOptions: any ): number {
        // console.log('AudioPlayer._playStart:', aOptions);
        if ( !this.mAudioBuffer ) {
            this.error( '_playStart', 'kein AudioBuffer vorhanden' );
            return -1;
        }
        if ( !this.mAudioContext ) {
