...
export class AudioPlayer extends Plugin implements AudioPlayerInterface {
    ...
    private mXMLHttpRequestClass: typeof XMLHttpRequest = null;
    ...
    private mAudioCancelFlag = false;

    // Events

    private mOnAudioStartFunc: OnAudioStartFunc = null;
    private mOnAudioStopFunc: OnAudioStopFunc = null;
    private mOnAudioUnlockFunc: OnAudioUnlockFunc = null;
    ...
    protected _detectXMLHttpRequest(): boolean {
        ...
        if ( !xmlHttpRequestFactory ) {
            this.error( '_detectXMLHttpRequest', 'keine XMLHttpRequest-Fabrik vorhanden' );
            return false;
        }
    ...
    protected _onAudioStart(): number {
        ...
        if ( typeof this.mOnAudioStartFunc === 'function' ) {
            try {
                ...
                this.exception( '_onAudioStart', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onAudioStop(): number {
        ...
        if ( typeof this.mOnAudioStopFunc === 'function' ) {
            try {
                ...
                this.exception( '_onAudioStop', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _onAudioUnlock(): number {
        ...
        if ( typeof this.mOnAudioUnlockFunc === 'function' ) {
            try {
                ...
                this.exception( '_onAudioUnlock', aException );
                return -1;
            }
        }
        return 0;
    ...
    protected _unlockAudio( aCallbackFunc: (aUnlockFlag: boolean) => void): void {
        ...
        if ( this.mAudioContext ) {
            ...
            if ( this.mAudioContext.state === 'suspended' || (this.mAudioContext as any).state === 'interrupted' ) {
                ...
                const timeoutId = setTimeout( () => { console.log('AudioPlayer._unlockAudio: timeout'); aCallbackFunc( false ); }, AUDIO_UNLOCK_TIMEOUT );
                // Resume aufrufen
    ...
    protected _loadAudioFile( aUrl: string ): number {
        // console.log('AudioPlayer._loadAudioFile:', aUrl);
        if ( !this.mXMLHttpRequestClass ) {
            this.error( '_loadAudioFile', 'keine XMLHttpRequest Klasse' );
            return -1;
        }
        try {
    ...
    protected _decodeAudio(): number {
        // console.log('AudioPlayer._decodeAudio');
        if ( !this.isInit()) {
            this.error( '_decodeAudio', 'nicht initialisiert' );
            return -1;
        }
        // pruefen auf Abbruch
        if ( this.isCancel()) {
        ...
        try {
            ...
            this._unlockAudio((aUnlockFlag: boolean) => {
                if ( aUnlockFlag ) {
                    ...
                    if ( !audioBuffer ) {
                        this.error( 'playByStream', 'kein Audiobuffer erzeugt');
                        return;
                    }
                    this.mAudioBuffer = audioBuffer;
                    this._playStart();
                } else {
                    this.error( 'play', 'AudioContext ist nicht entsperrt' );
                    this._onAudioStop();
                }
            });
            return 0;
        } catch (aException) {
            this.exception( 'play', aException );
            return -1;
        }
    }
    ...
    play( aAudioFilePath: string, aAudioId: string ): number {
        ...
        try {
            ...
            if ( this.mAudioContext.state === 'suspended' ) {
                this.error( 'playFile', 'AudioContext ist nicht entsperrt' );
                this._onAudioStop();
                return -1;
            }
    ...
    playFile( aFileName: string ): number {
        ...
        if ( !aFileName ) {
            this.error( 'playFile', 'kein Dateiname uebergeben' );
            return -1;
        }
        ...
        try {
            ...
            this.exception( 'playFile', aException );
            return -1;
        }
    ...
    protected _getAudioBufferFirst( aData: any ): AudioBuffer {
    ...
    protected _getAudioBufferSecond( aData: any ): AudioBuffer {
        let audioBuffer: AudioBuffer = null;
        // fuer die Browser ohne AudioBuffer Constructor
        try {
            const audioToPlay = new Float32Array( aData.length );
    ...
    protected _getAudioBufferResample( aData: any ): AudioBuffer {
        let audioBuffer: AudioBuffer = null;
        // Fuer Safari wird eine hoehere SampleRate als 16000 benoetigt, da createBuffer sonst nicht funktioniert
        // hier wird der Resampler eingesetzt
        try {
