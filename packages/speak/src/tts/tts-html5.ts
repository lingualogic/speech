...
export class TTSHtml5 extends TTSPlugin {
    ...
    private mSynthesisFactory: SpeechSynthesisFactory;
    ...
    private mUtteranceClass: any = null;
    ...
    constructor( aPluginName?: string, aRegisterFlag = true ) {
        ...
        this.mSynthesisFactory.setErrorOutputFunc( this._getErrorOutputFunc());
    }
    ...
    setErrorOutput( aErrorOutputFlag: boolean ): void {
        if ( this.mSynthesisFactory ) {
            this.mSynthesisFactory.setErrorOutput( aErrorOutputFlag );
        }
    ...
    protected _breakSynthesis(): void {
        // console.log('TTSHtml5._beakSynthesis: Start Timeout');
        this.mBreakTimeout = 0;
        this.error( '_breakSynthesis', 'Kein SpeechSynthesis-Service vorhanden' );
        this.stopSpeak();
    }
    ...
    getVoiceList(): Array<string> {
        if ( !this.mSynthesis ) {
            this.error( 'getVoiceList', 'keine SpeechSynthesis vorhanden' );
            return [];
        }
        // pruefen auf vorhandene Sprache
        const language = this._getTTSLanguage();
        if ( !language ) {
            this.error( 'getVoiceList', 'keine Sprache vorhanden' );
            return [];
        }
        ...
        try {
            ...
            if ( !Array.isArray( voiceList )) {
                this.error( 'getVoiceList', 'keine Voice-Liste als Array vorhanden' );
                return [];
            }
            ...
            this.exception( 'getVoiceList', aException );
            return [];
        }
    }
    ...
    protected _getTTSVoice( aLocalServiceFlag = true ): any {
        if ( !this.mSynthesis ) {
            this.error( '_getTTSVoice', 'keine SpeechSynthesis vorhanden' );
            return undefined;
        }
        // pruefen auf vorhandene Sprache
        const language = this._getTTSLanguage();
        if ( !language ) {
            this.error( '_getTTSVoice', 'keine Sprache vorhanden' );
            return undefined;
        }
        // Stimme auswaehlen
        ...
        try {
            ...
            // nicht alle Implementierungen von SpeechSynthesis liefern ein Array von SpeechSynthesisVoice Objekten
            if ( !Array.isArray( voiceList )) {
                this.error( '_getTTSVoice', 'keine Voice-Liste als Array vorhanden' );
                return undefined;
            }
            if ( voice ) {
            ...
            this.exception( '_getTTSVoice', aException );
            return undefined;
        }
    }
    ...
    protected _detectSynthesis(): boolean {
        ...
        if ( !this.mSynthesisFactory ) {
            this.error( '_detectSynthesis', 'keine SpeechSynthesis-Fabrik vorhanden' );
            return false;
        }
    ...
    protected _createSynthesis( aText: string ): number {
        try {
            ...
            this.exception( '_createSynthesis', aException );
            return -1;
        }
    }
    ...
    protected _probeSynthesis(): number {
        ...
        try {
            ...
            if ( !utterance ) {
                this.error( '_probeSynthesis', 'Kein Utterance erzeugt' );
                return -1;
            }
            ...
            this.exception( '_probeSynthesis', aException );
            return -1;
        }
    }
