...
import { INTENT_API_VERSION, INTENT_VERSION_STRING } from '../intent-version';
...
export class IntentComponent extends BaseComponent implements IntentComponentInterface {
    ...
    private mNLUPlugin: NLUInterface = null;

    // Events

    private mListenResultEvent = new EventFunctionList( SPEECH_LISTENRESULT_EVENT, INTENT_COMPONENT_NAME );
    private mIntentResultEvent = new EventFunctionList( SPEECH_INTENTRESULT_EVENT, INTENT_COMPONENT_NAME );

    // Attribute der Komponente

    private mASRActiveFlag = false;
    private mNLUActiveFlag = false;

    // Features des Servers

    private mNLUFeatureFlag = false;
    ...
    constructor( aComponentName = '', aRegisterFlag = true ) {
        super( aComponentName || INTENT_COMPONENT_NAME, aRegisterFlag );
        this.mListenResultEvent.setErrorOutputFunc( this._getErrorOutputFunc());
        this.mIntentResultEvent.setErrorOutputFunc( this._getErrorOutputFunc());
    }
    ...
    getVersion(): string {
        return INTENT_VERSION_STRING;
    }


    getApiVersion(): string {
        return INTENT_API_VERSION;
    }
    ...
    setErrorOutput( aErrorOutputFlag: boolean ): void {
        ...
        this.mIntentResultEvent.setErrorOutput( aErrorOutputFlag );
    }
    ...
    protected _onListenResult( aText: string ): number {
    ...
    protected _onIntentResult( aIntent: any ): number {
    ...
    insertNLU( aNLUName: string, aNLUType: string, aOption: any ): number {
        ...
        return this.mNLUPlugin.insertNLU( aNLUName, aNLUType, aOption );
    }
    ...
    removeNLU( aNLUName: string ): number {
        ...
        return this.mNLUPlugin.removeNLU( aNLUName );
    }
    ...
    start(): number {
        ...
        if ( !this.isNLU()) {
            this.error( 'start', 'keine NLU vorhanden' );
            return -1;
        }
        ...
        if ( !this.mIntentText ) {
            if ( this.mNLUPlugin.isListen()) {
                ...
                this.error( 'start', 'Keinen Text zur Sprachanalyse uebergeben' );
                return -1;
            }
        }
    ...
    stop(): number {
        ...
        if ( !this.mNLUPlugin ) {
            this.error( 'stop', 'keine NLU vorhanden' );
            return -1;
        }
    ...
    abort(): number {
        ...
        if ( !this.mNLUPlugin ) {
            this.error( 'abort', 'keine NLU vorhanden' );
            return -1;
        }
        // interne NLU verwenden
        if ( !this.mIntentText ) {
