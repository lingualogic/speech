...
export class NLUGroup extends PluginGroup implements NLUInterface {
    ...
    private mNLUFactory: NLUFactory = null;


    // alle inneren NLU

    private mNLUHtml5: NLUInterface = null;
    private mNLUNuance: NLUInterface = null;
    private mNLUGoogle: NLUInterface = null;
    private mNLUMicrosoft: NLUInterface = null;
    private mNLURasa: NLUInterface = null;
    private mNLUList = new PluginList();

    // aktuell genutzte NLU

    private mCurrentNLU: NLUInterface = null;
    ...
    private mOnConnectFunc: OnNLUConnectFunc = null;
    ...
    private mOnDisconnectFunc: OnNLUDisconnectFunc = null;
    ...
    private mOnListenStartFunc: OnNLUListenStartFunc = null;
    ...
    private mOnListenStopFunc: OnNLUListenStopFunc = null;
    ...
    private mOnListenResultFunc: OnNLUListenResultFunc = null;
    ...
    private mOnIntentResultFunc: OnNLUIntentResultFunc = null;
    ...
    protected _insertAllNLU(): void {
        // pruefen auf NLU-Fabrik
        if ( !this.mNLUFactory ) {
            this.error( '_insertAllNLU', 'keine NLU-Fabrik vorhanden' );
            return;
        }
        // eintragen der verfuegbaren NLU-Plugins
        this.insertPlugin( NLU_NUANCE_NAME, this.mNLUFactory.create( NLU_NUANCE_NAME, NLU_NUANCE_NAME, false ));
        this.insertPlugin( NLU_GOOGLE_NAME, this.mNLUFactory.create( NLU_GOOGLE_NAME, NLU_GOOGLE_NAME, false ));
        this.insertPlugin( NLU_MICROSOFT_NAME, this.mNLUFactory.create( NLU_MICROSOFT_NAME, NLU_MICROSOFT_NAME, false ));
        this.insertPlugin( NLU_RASA_NAME, this.mNLUFactory.create( NLU_RASA_NAME, NLU_RASA_NAME, false ));
        // TODO: Html5 erst eintragen, wenn Grammatik programmiert wurde
        // this.insertPlugin( NLU_HTML5_NAME, this.mNLUFactory.create( NLU_HTML5_NAME, NLU_HTML5_NAME, false ));
    }
    ...
    protected _initNLUHtml5( aOption: any ): void {
    ...
    protected _initNLUNuance( aOption: any ): void {
    ...
    protected _initNLUGoogle( aOption: any ): void {
    ...
    protected _initNLUMicrosoft( aOption: any ): void {
    ...
    protected _initNLURasa( aOption: any ): void {
    ...
    init( aOption?: any ): number {
        ...
        if ( !this.mNLUFactory ) {
            this.error( 'init', 'keine NLU-Fabrik vorhanden' );
            return -1;
        }
    ...
    set onIntentResult( aOnIntentResultFunc: OnNLUIntentResultFunc ) {
        this.mOnIntentResultFunc = aOnIntentResultFunc;
        let nlu = this.firstPlugin() as NLUInterface;
        while ( nlu ) {
     ...
     * @param aNLUClass - Klassenname der NLU
     * @param aOption - optionale Parameter zur Initialisierung der NLU
     */

    insertNLU( aNLUName: string, aNLUClass: string, aOption: any ): number {
        // pruefen auf vorhandenes Plugin
        if ( this.findPlugin( aNLUName )) {
            return 0;
        }
        // NLU-Plugin eintragen
        // TODO: Untersuchen, warum ich hier RegisterFlag = false gemacht habe
        if ( this.insertPlugin( aNLUName, this.mNLUFactory.create( aNLUName, aNLUClass, false )) !== 0 ) {
            return -1;
        }
        // NLU-Plugin initialisieren
        const nluPlugin = this.findPlugin( aNLUName ) as NLUInterface;
        if ( nluPlugin ) {
            nluPlugin.init( aOption );
            if ( nluPlugin.isActive()) {
                if ( this.isErrorOutput()) {
                    console.log('NLUGroup.insertNLU: NLU eingefuegt ', aNLUName, aNLUClass);
                }
                // Funktionen verbinden
                nluPlugin.onInit = this.mOnInitFunc;
                nluPlugin.onListenStart = this.mOnListenStartFunc;
                nluPlugin.onListenStop = this.mOnListenStopFunc;
                nluPlugin.onListenResult = this.mOnListenResultFunc;
                nluPlugin.onIntentResult = this.mOnIntentResultFunc;
                nluPlugin.onError = this.mOnErrorFunc;
                return this.mNLUList.insert( aNLUName, nluPlugin );
            }
            this.removePlugin( aNLUName );
        }
        if ( this.isErrorOutput()) {
            console.log('NLUGroup.insertNLU: NLU nicht eingefuegt ', aNLUName, aNLUClass);
        }
        return -1;
    }
    ...
    removeNLU( aNLUName: string ): number {
        // NLU-Plugin initialisieren
        const nluPlugin = this.mNLUList.find( aNLUName );
        if ( nluPlugin ) {
            this.mNLUList.remove( aNLUName );
            return this.removePlugin( aNLUName );
        }
        if ( this.isErrorOutput()) {
            console.log('NLUGroup.removeNLU: NLU nicht vorhanden');
        }
        return -1;
    }
    ...
    setNLU( aNLUName: string ): number {
        ...
        switch ( aNLUName ) {
            ...
            default:
                // individuelle NLUs suchen
                nlu = this.mNLUList.find( aNLUName );
                break;
        }
        ...
        if ( !nlu ) {
            this.error( 'setNLU', 'Keine NLU vorhanden' );
            return -1;
        }
    ...
    setLanguage( aLanguage: string ): number {
        ...
        if ( !nlu ) {
            this.error( 'setLanguage', 'Keine NLU vorhanden' );
            return -1;
        }
        while ( nlu ) {
    ...
    startIntent( aText: string ): number {
        ...
        if ( !this.mCurrentNLU ) {
            this.error( 'startIntent', 'keine NLU vorhanden' );
            return -1;
        }
    ...
    startListen(): number {
        // pruefen auf aktive Komponente
        if ( !this.mCurrentNLU ) {
            this.error( 'startListen', 'keine NLU vorhanden' );
            return -1;
        }
        if ( !this.isActive()) {
            // kein Fehler
