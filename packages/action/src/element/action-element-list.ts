...
export class ActionElementList extends ErrorBase {
    ...
    private mActionFuncList = new Map<string, [ActionStartFunc, ActionStopFunc]>();
    ...
    insert( aElementName: string, aActionStartFunc: ActionStartFunc, aActionStopFunc: ActionStopFunc ): number {
        ...
        if ( !aElementName ) {
            ...
            this.error( 'insert', 'kein Elementname uebergeben' );
            return -1;
        }
        if ( this.mActionFuncList.get( aElementName )) {
            this.error( 'insert', 'Element bereits eingetragen' );
            return -1;
        }
        if ( typeof aActionStartFunc !== 'function' ) {
            this.error( 'insert', 'keine ActionStart-Funktion uebergeben' );
            return -1;
        }
        if ( typeof aActionStopFunc !== 'function' ) {
            this.error( 'insert', 'keine ActionStop-Funktion uebergeben' );
            return -1;
        }
    ...
    remove( aElementName: string ): number {
        if ( !aElementName ) {
            this.error( 'remove', 'kein Action-Elementname uebergeben' );
            return -1;
        }
        try {
    ...
    getStartAction( aElementName: string ): ActionStartFunc {
        try {
            ...
            this.exception( 'getStartAction', aException );
        }
        // leere Funktion zurueckgeben
        this.error( 'getStartAction', 'keine Funktion vorhanden' );
    ...
    getStopAction( aElementName: string ): ActionStopFunc {
        try {
            ...
            this.exception( 'getStopAction', aException );
        }
        // leere Funktion zurueckgeben
        this.error( 'getStopAction', 'keine Funktion vorhanden' );
        return () => 0;
    }
    ...
    getActionTuple( aElementName: string ): [ActionStartFunc, ActionStopFunc] {
        try {
            ...
            this.exception( 'getActionTupel', aException );
            return null;
        }
    }
    ...
    startAction(aAction: ActionDataInterface): number {
        ...
        try {
            ...
            if ( typeof startActionFunction !== 'function' ) {
                this.error( 'startAction', 'keine StartAction-Funktion vorhanden' );
                return -1;
            }
            ...
            if ( typeof stopActionFunction !== 'function' ) {
                this.error( 'startAction', 'keine StopAction-Funktion vorhanden' );
                return -1;
            }
