...
import { ACTION_API_VERSION, ACTION_VERSION_STRING } from '../action-version';
...
export class ActionComponent extends BaseComponent implements ActionComponentInterface {
    ...
    private mActionFunction: ActionFunctionInterface = null;
    private mActionElement: ActionElementInterface = null;


    // interne Attribute

    private mActionRunningFlag = false;
    private mActionName = '';                          // Name der auszufuehrenden Aktion
    private mActionElementType = '';                   // Name des Elementtyps der Aktion
    private mActionElementName = '';                   // Name des Elements der Aktion
    private mActionTimeout = ACTION_MAX_TIMEOUT;       // Aktions-Timeout 10 sekunden
    private mActionTimeoutId = 0;                      // Timeout-ID
    ...
    constructor( aComponentName = '', aRegisterFlag = true ) {
        super( aComponentName || ACTION_COMPONENT_NAME, aRegisterFlag );
    }
    ...
    getVersion(): string {
        return ACTION_VERSION_STRING;
    }


    getApiVersion(): string {
        return ACTION_API_VERSION;
    }
   ...
   protected _initAllPlugin(): number {
    ...
    getActionFunc(): ActionFunc {
        return (aActionData: ActionDataInterface) => this.startAction( aActionData );
    }
    ...
    startAction( aActionData: ActionDataInterface ): number {
        ...
        if ( this.isRunning()) {
            this.error( 'startAction', 'Aktion laeuft bereits' );
            return -1;
        }
    ...
    start(): number {
        if ( !this.mActionName ) {
            this.error( 'startAction', 'kein Aktionsname vorhanden' );
            return -1;
        }
        if ( !this.mActionElementName ) {
            this.error( 'startAction', 'kein Elementname vorhanden' );
            return -1;
        }
        ...
        return this.startAction( actionData );
    }
    ...
    addFunction( aFunctionName: string, aStartActionFunc: ActionStartFunc, aStopActionFunc?: ActionStopFunc ): number {
        if ( !this.mActionFunction ) {
            this.error( 'addFunction', 'kein ActionFunction-Plugin vorhanden' );
            return -1;
        }
    ...
    removeFunction( aFunctionName: string ): number {
        if ( !this.mActionFunction) {
            this.error( 'removeFunction', 'kein ActionFunction-Plugin vorhanden' );
            return -1;
        }
    ...
    addElement( aElementName: string, aStartActionFunc: ActionStartFunc, aStopActionFunc: ActionStopFunc ): number {
        if ( !this.mActionElement ) {
            this.error( 'addElement', 'kein ActionElement-Plugin vorhanden' );
            return -1;
        }
    ...
    removeElement( aElementName: string ): number {
        if ( !this.mActionElement ) {
            this.error( 'removeElement', 'kein ActionElement-Plugin vorhanden' );
            return -1;
        }
